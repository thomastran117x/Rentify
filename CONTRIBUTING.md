...
Follow the [PR writing guide](./docs/pull-requests.md) and use the [default](./.github/PULL_REQUEST_TEMPLATE.md), [feature](./.github/PULL_REQUEST_TEMPLATE/feature.md), or [bug](./.github/PULL_REQUEST_TEMPLATE/bug.md) template. Include Summary, Context, Changes, API, How to Test, and Reviewer Notes; explicitly mark sections with no applicable changes. Explain what was missing/broken and what improved, affected routes/contracts, user- or consumer-level test steps, risks, migrations/configuration, and commit dependencies. Do not duplicate routine CI commands or results in the PR description.

For visual changes to core frontend pages/flows, add actual screenshots with state and viewport captions when capture is available. Include before/after or responsive/error states when useful. If screenshots are unavailable, explain the limitation and provide capture steps or local artifacts. Non-visual changes can mark screenshots not applicable; screenshots do not replace browser validation.

Review the full diff and commit sequence before handoff. In the implementation handoff, record commands and outcomes, Docker/browser validation ownership, unavailable checks, and any ordering dependencies between commits. Keep that engineering evidence out of the PR's How to Test section unless it is exceptional information that affects review. Treat blockers honestly and distinguish completed checks from checks still needed.
