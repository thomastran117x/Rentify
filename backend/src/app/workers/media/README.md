...
2. Reads the properties of `quarantine/images/<userId>/<mediaId>` before downloading it. An empty blob, one over the size limit, and one whose ETag differs from the one recorded at completion are rejected without being downloaded. Any write after completion changes the ETag, even a re-upload of identical bytes, so that is rejected too. Rows completed before the ETag was recorded skip the ETag comparison and keep the size checks.
3. Downloads the blob under `If-Match` on that ETag, asking for at most one byte past the size limit, and checks its real length, format, and dimensions with the image policy, against the type declared when the upload started and today's allow-list. A blob that changes or grows past the limit during the download is rejected.
4. Re-encodes an accepted image to WebP at `media/images/<userId>/<mediaId>.webp`. Re-encoding applies the EXIF orientation and drops metadata such as EXIF and GPS. It then marks the row `ready` and deletes the quarantined upload.
5. Marks a row whose bytes fail the policy (413, 415, or 422), whose upload is missing, or whose upload changed after completion `rejected` with the reason, and deletes the quarantined upload. These are final, and they are not retried. A failure to read the properties or the bytes, such as storage being unavailable, is retried.
