...
const workerName = "Organization search maintainer worker";
const workerResources = searchBrokerWorkerResources;
const MIN_IDLE_SLEEP_MS = 100;
const workerLogger = loggerFactory
  .forComponent("organization-search-maintainer.worker", "worker")
  .child({
    workerName,
  });

export async function bootstrapOrganizationSearchMaintainerWorker(): Promise<void> {
...
async function runScheduledTask(
  ...
  try {
    ...
    workerLogger.error(
      "Scheduled organization search maintenance task failed.",
      {
        parentWorkerName,
        taskName,
      },
...
startWorker({
  name: workerName,
  bootstrap: bootstrapOrganizationSearchMaintainerWorker,
