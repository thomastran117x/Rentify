...
async function hashPasswords(): Promise<Map<string, string>> {
  const values = new Set(
    SEED_USERS.map((user) => user.password).filter(
...
export const usersSeedModule: SeedModule = {
  ...
  async run({ logger, prisma, refresh, state }) {
    ...
    for (const fixtureUser of SEED_USERS) {
      const passwordHash =
        fixtureUser.password === null
          ? null
          : (passwordHashes.get(fixtureUser.password) ?? null);

      if (fixtureUser.password !== null && !passwordHash) {
      ...
      await prisma.profile.upsert({
        ...
        update: {
          ...
          usernameChangedAt,
          usernameAutoGenerated: fixtureUser.usernameAutoGenerated ?? false,
