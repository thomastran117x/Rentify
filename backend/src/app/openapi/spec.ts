...
const checkoutSummaryExample = {
  serverTime: "2026-05-25T12:00:00.000Z",
  booking: {
    id: "booking-1",
    status: "awaiting_payment",
    startAt: "2026-06-14T15:00:00.000Z",
  ...
  payment: null,
  paypal: {
    clientId: "example-sandbox-client-id",
    environment: "sandbox",
  ...
  return [
    ...
    },
    {
      ...
      description: [
        ...
        "",
        "409 responses carry `error.details.reason`: `payment_in_progress` (the earlier order was already approved or paid and has been applied), `checkout_busy` (another checkout request for this booking is in flight; retry), or `reconciliation_required`. Replaying an idempotency key returns the original order; reusing it with a different method is a 409.",
        "",
        "Provider failures are recorded on the returned payment instead of failing the request. PAT bearer authentication is not allowed on payment-session routes.",
      ].join("\n"),
      tags: ["payments"],
      security: ownerSecurity,
      permissions: {
    ...
    {
      method: "get",
      path: "/booking-requests/:id/checkout",
      operationId: "getBookingCheckoutSummary",
      summary: "Get the checkout summary for a booking request",
      description:
        "Returns what the renter sees before paying: booking dates and guests, the price breakdown (stay total, deposit charged today, platform fee, total due now, and the remaining balance Rentify does not charge), the enforced cancellation policy with the host's notes, whether checkout can start, the current payment if one exists, and the PayPal settings the JS SDK needs. When a payment exists, its stored amounts are reported because they are what gets charged; `depositBps` and `platformFeeBps` are null when those amounts predate the current pricing. Only the renter may read it; organization members get 403. PAT bearer authentication is not allowed.",
      tags: ["payments"],
      security: ownerSecurity,
      permissions: {
        authMode: "session-bearer",
    ...
    },
    {
      ...
      description: [
        "Captures the PayPal order the renter approved at checkout and finalizes the payment. Only the renter or a member who can manage the organization's payments may capture. Safe to repeat: an already captured payment is returned unchanged.",
        "",
        "Pass the approved `orderId` from the JS SDK or the return URL; if a newer checkout replaced that order, nothing is captured and the response is 409 with `error.details.reason` `stale_order`. The order is read from PayPal first and is only captured when it is approved, the booking hold is still live, the order matches this payment's amount and currency, and card orders passed 3-D Secure. An order that is not approved yet leaves the payment unchanged. A refused order ends the attempt with `HOLD_EXPIRED`, `ORDER_MISMATCH`, `CARD_AUTHENTICATION_FAILED` or `CARD_AUTHENTICATION_UNAVAILABLE` and returns the failed payment so the renter can retry.",
        "",
        "Other 409 reasons: `reconciliation_required`, `checkout_busy`. PAT bearer authentication is not allowed.",
      ].join("\n"),
      tags: ["payments"],
      security: ownerSecurity,
      permissions: {
      ...
      parameters: [routePathParam("id", "Payment identifier.", "payment-1")],
      requestBody: {
        ...requestBody("CapturePaymentRequest", {
          orderId: "5O190127TN364715T",
        }),
        required: false,
      },
      ...
      CapturePaymentRequest: {
        ...
        properties: {
          orderId: {
            ...
            maxLength: 128,
            description:
              "The PayPal order the renter approved. Rejected with 409 when it is no longer the payment's current order.",
          },
        },
      },
      CheckoutSummary: {
        ...
        properties: {
          ...
          pricing: {
            ...
            properties: {
              currency: { type: "string" },
              stayTotal: { type: "number" },
              depositAmount: { type: "number" },
              platformFeeAmount: { type: "number" },
              totalDueNow: {
                type: "number",
                description: "The amount charged at checkout.",
              },
              remainingBalance: {
                type: "number",
                description:
          ...
          cancellationPolicy: {
            ...
            required: [
              ...
              "partialRefundCutoffHours",
              "partialRefundPercent",
              "ownerCancellationFullRefund",
              "refundBase",
            ],
            properties: {
              code: { type: "string" },
              fullRefundCutoffHours: { type: "integer" },
              partialRefundCutoffHours: { type: "integer" },
              partialRefundPercent: { type: "integer" },
              ownerCancellationFullRefund: { type: "boolean" },
              refundBase: { type: "string", enum: ["total_paid"] },
          ...
          paypal: {
            type: "object",
            required: ["clientId", "environment", "enabledMethods"],
            properties: {
