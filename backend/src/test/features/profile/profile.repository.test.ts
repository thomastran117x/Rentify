...
function createUpdateMock() {
  return jest.fn(async (_args: { data: Record<string, unknown> }) =>
...
describe("ProfileRepository", () => {
  ...
  it("updates and maps profile persistence fields", async () => {
    ...
    // Non-null: only a guarded rename that loses a race returns null, and this
    // update carries no guard.
  ...
  it("derives the cooldown state from the stored rename timestamp", async () => {
    const changedAt = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const findUnique = jest.fn(async () =>
      createProfilePersistence({ usernameChangedAt: changedAt }),
    );
    const repository = new ProfileRepository({
      profile: { findUnique },
    } as any);
    ...
    expect(result?.usernameChangeAvailableAt).toBe(
      new Date(changedAt.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString(),
    );
  });
  ...
  describe("guarded renames", () => {
    it("tests eligibility inside the write rather than trusting the caller's read", async () => {
      const guardAt = new Date("2026-08-17T12:00:00.000Z");
      const updateMany = jest.fn(async (_args: unknown) => ({ count: 1 }));
      const findUnique = jest.fn(async () => createProfilePersistence());
      const repository = new ProfileRepository({
        profile: { updateMany, findUnique },
      } as any);
      ...
      expect(args.where.OR).toEqual([
        ...
        {
          usernameChangedAt: {
            lte: new Date(guardAt.getTime() - 30 * 24 * 60 * 60 * 1000),
          },
        },
      ]);
    });

    it("returns null when the guarded write matches no row", async () => {
      // A concurrent rename stamped the clock between the caller's read and
      // this write, so the WHERE clause no longer matches.
    ...
    it("does not use the guarded path for an ordinary save", async () => {
      const update = createUpdateMock();
      const updateMany = jest.fn(async (_args: unknown) => ({ count: 1 }));
      const repository = new ProfileRepository({
  ...
  it("writes the cooldown columns when the service supplies them", async () => {
    const changedAt = new Date("2026-08-17T12:00:00.000Z");
    const update = createUpdateMock();
    const repository = new ProfileRepository({
      profile: { update },
    } as any);
    ...
    const data = update.mock.calls[0]?.[0].data ?? {};
    expect(data.usernameChangedAt).toBe(changedAt);
    expect(data.usernameAutoGenerated).toBe(false);
  });
});
