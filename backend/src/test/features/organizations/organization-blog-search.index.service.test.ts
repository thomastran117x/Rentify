...
import type { OrganizationBlogSearchDocument } from "@/features/organizations/organization-blog.model";
...
describe("OrganizationBlogSearchIndexService", () => {
  ...
  it("creates a concrete index and wires aliases when none exist", async () => {
    ...
    // The mapping includes blog-specific fields.
    const indexBody = JSON.parse((putIndex![1] as { body: string }).body) as {
      mappings: { properties: Record<string, unknown> };
    };
    expect(indexBody.mappings.properties.title).toBeDefined();
    expect(indexBody.mappings.properties.status).toBeDefined();
    expect(indexBody.mappings.properties.organizationId).toBeDefined();
  });
  ...
  it("reports a ready alias status when both aliases share one index", async () => {
    const handler: RequestHandler = (path) => {
      if (path.includes("-read") || path.includes("-write")) {
        return { "postings-organization-blogs_v1": {} };
      }
      return {};
    };
    ...
    expect(status.readTargets).toEqual(["postings-organization-blogs_v1"]);
  });
  ...
  it("bulk deletes documents and ignores 404s while surfacing real errors", async () => {
    ...
    const failing = new OrganizationBlogSearchIndexService(
  ...
  it("upserts and deletes a single document against the write alias", async () => {
    ...
    const put = requestJson.mock.calls.find(
      ([path, init]) =>
        String(path).includes("/_doc/post-1") &&
        (init as { method?: string }).method === "PUT",
    );
    ...
    expect(body.title).toBe("Post post-1");
    expect(body.status).toBe("published");
    expect(body.organizationId).toBe("org-post-1");
  });
  ...
  it("bulk upserts documents and surfaces bulk errors", async () => {
    ...
    expect((bulkCall![1] as { body: string }).body).toContain("Post a");
  ...
  it("swaps aliases from the previous targets to a new index", async () => {
    ...
    const result = await service.swapAliases("postings-organization-blogs_v2");

    expect(result.previousReadTargets).toEqual([
      "postings-organization-blogs_v1",
    ]);
    const aliasCall = requestJson.mock.calls.find(
      ([path]) => path === "/_aliases",
    ...
    expect(
      body.actions.some(
        (action) => action.add?.index === "postings-organization-blogs_v2",
      ),
    ).toBe(true);
  ...
  it("creates a versioned index and tolerates an already-existing index", async () => {
    ...
    const name = await service.createVersionedIndex(
      "postings-organization-blogs_v9",
    );
    expect(name).toBe("postings-organization-blogs_v9");
    ...
    const conflictService = new OrganizationBlogSearchIndexService(
      conflictClient.client as any,
    );
    await expect(
      conflictService.createVersionedIndex("postings-organization-blogs_v9"),
    ).resolves.toBe("postings-organization-blogs_v9");
  });
  ...
  it("deletes a concrete index", async () => {
    ...
    await service.deleteConcreteIndex("postings-organization-blogs_v1");

    const del = requestJson.mock.calls.find(
      ([path, init]) =>
        path === "/postings-organization-blogs_v1" &&
        (init as { method?: string }).method === "DELETE",
    );
    expect(del).toBeDefined();
  });
