...
function createHarness(overrides?: {
  ...
  const repository = {
    ...
    countBlogPostsForIndexing: jest.fn(async () => 2),
  ...
  const indexService = {
    ...
    getAliasStatus: jest.fn(async () => ({
      state: "ready",
      readAlias: "organization-blogs-read",
      writeAlias: "organization-blogs-write",
      readTargets: ["organization-blogs_v1"],
      writeTargets: ["organization-blogs_v1"],
    })),
    getReadAliasName: () => "organization-blogs-read",
    getWriteAliasName: () => "organization-blogs-write",
    ...
    buildVersionedIndexName: () => "organization-blogs_v2",
    ...
    createVersionedIndex: jest.fn(async () => "organization-blogs_v1"),
    swapAliases: jest.fn(async () => ({
      previousReadTargets: ["organization-blogs_v0"],
      previousWriteTargets: ["organization-blogs_v0"],
    })),
  ...
  const service = new OrganizationBlogSearchService(
    repository as any,
    indexService as any,
    queueService as any,
  );
...
describe("OrganizationBlogSearchService", () => {
  describe("processIndexJob", () => {
    ...
    it("deletes when the blog post no longer exists", async () => {
      const { service, indexService } = createHarness({
    ...
    it("marks a barrier job as indexed without touching the index", async () => {
      const { service, repository, indexService } = createHarness({
        repository: {
          getSearchOutboxById: jest.fn(async () =>
            createOutbox({ operation: "barrier", blogPostId: undefined }),
          ),
        },
      });
  ...
  describe("processIndexJobsBatch", () => {
    ...
    it("bulk deletes when the blog post row is gone", async () => {
      const { service, indexService } = createHarness({
        repository: {
          getSearchOutboxesByIds: jest.fn(async () => [createOutbox()]),
  ...
  describe("processOutboxRelayBatch", () => {
    ...
    it("carries the blog post id in the relayed job payload", async () => {
    ...
    it("coalesces multiple pending jobs for the same blog post", async () => {
      const { service, queueService, repository } = createHarness({
        repository: {
          claimSearchOutboxBatch: jest.fn(async () => [
            createOutbox({ id: "o1" }),
  ...
  describe("reindex lifecycle", () => {
    it("starts a reindex run when none is active", async () => {
      ...
      expect(run.targetIndexName).toBe("organization-blogs_v2");
    });
    ...
    it("swaps aliases once a catch-up run is complete", async () => {
      ...
      expect(indexService.swapAliases).toHaveBeenCalledWith(
    ...
    it("marks the run failed on a non-transient error", async () => {
      const { service, repository } = createHarness({
        repository: {
          ...
          countBlogPostsForIndexing: jest.fn(async () => {
  ...
  describe("status and maintenance", () => {
    it("reports pipeline status", async () => {
      ...
      expect(status.aliases.read).toBe("organization-blogs-read");
    ...
    it("reconciles recent blog posts into the index", async () => {
      const { service, indexService } = createHarness();
      ...
      const { service, indexService } = createHarness({
        repository: {
          listCompletedSearchReindexRunsWithRetainedIndices: jest.fn(
            async () => [
              createReindexRun({ retainedIndexName: "organization-blogs_v0" }),
            ],
          ),
        },
      });
