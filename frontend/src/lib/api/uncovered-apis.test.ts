...
describe("previously uncovered API helpers", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildPathMock.mockImplementation(
      (path: string, query: Record<string, unknown>) =>
        `${path}?${new URLSearchParams(
          Object.entries(query)
            .filter(([, value]) => value !== undefined)
  ...
  it("uses public endpoints for system health and profile listing", () => {
    ...
    expect(publicJsonMock).toHaveBeenLastCalledWith(
      "GET",
      "/profiles?page=2&pageSize=50&q=alex",
    );
  });
  ...
  it("uses default pagination and encoded paths for saved postings", () => {
    ...
    expect(authenticatedJsonMock).toHaveBeenNthCalledWith(
      1,
      "POST",
      "/postings/posting%20%2F%201/save",
    );
    expect(authenticatedJsonMock).toHaveBeenNthCalledWith(
      2,
      "DELETE",
    ...
    expect(authenticatedJsonMock).toHaveBeenNthCalledWith(
      3,
      "GET",
      "/postings/saved?page=1&pageSize=20",
    );
    expect(authenticatedJsonMock).toHaveBeenNthCalledWith(
      4,
      "GET",
  ...
  it("delegates feedback, feature flag, search administration, and token requests", () => {
    ...
    personalAccessTokensApi.create({
      name: "CLI",
      expiresInDays: 30,
      scopes: ["mcp:read"],
    });
    ...
    expect(authenticatedJsonMock).toHaveBeenCalledWith(
      "PUT",
      "/admin/feature-flags/search%20beta",
      { enabled: true },
    );
    expect(authenticatedJsonMock).toHaveBeenCalledWith(
      "DELETE",
    ...
    expect(authenticatedJsonMock).toHaveBeenCalledWith(
      "GET",
      "/admin/search/reindex/run%20%2F%201",
    );
    expect(authenticatedJsonMock).toHaveBeenCalledWith(
      "POST",
    ...
    expect(authenticatedJsonMock).toHaveBeenCalledWith(
      "DELETE",
      "/auth/personal-access-tokens/token%20%2F%201",
    );
  });
});
