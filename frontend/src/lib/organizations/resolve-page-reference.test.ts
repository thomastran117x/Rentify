...
const { resolveMock, canonicalMock, hrefMock, redirectMock, permanentRedirectMock } = vi.hoisted(() => ({
  resolveMock: vi.fn(),
  canonicalMock: vi.fn(),
  hrefMock: vi.fn(),
  redirectMock: vi.fn(),
...
describe("resolveOrganizationPageReference", () => {
  it("returns missing and canonical references without redirecting", async () => {
    ...
    await expect(resolveOrganizationPageReference("missing")).resolves.toBeNull();

    const resolved = { organizationId: "org-1", canonicalSlug: "studio", name: "Studio", matchedBy: "canonical-slug" };
    resolveMock.mockResolvedValueOnce(resolved);
    canonicalMock.mockReturnValueOnce(true);
    await expect(resolveOrganizationPageReference("studio", "blog")).resolves.toEqual(resolved);
  });

  it("uses temporary UUID redirects and permanent alias redirects", async () => {
    hrefMock.mockReturnValue("/organizations/studio/blog");
    resolveMock.mockResolvedValueOnce({ organizationId: "org-1", canonicalSlug: "studio", name: "Studio", matchedBy: "uuid" });
    ...
    resolveMock.mockResolvedValueOnce({ organizationId: "org-1", canonicalSlug: "studio", name: "Studio", matchedBy: "alias" });
    canonicalMock.mockReturnValueOnce(false);
    await resolveOrganizationPageReference("old-studio", "blog");
    expect(permanentRedirectMock).toHaveBeenCalledWith("/organizations/studio/blog");
  });
});
