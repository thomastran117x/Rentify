...
const { editor, chain, useEditorMock } = vi.hoisted(() => {
  ...
  ["focus", "toggleBold", "toggleItalic", "toggleStrike", "toggleHeading", "toggleBulletList", "toggleOrderedList", "toggleBlockquote", "toggleCodeBlock", "extendMarkRange", "unsetLink", "setLink", "setImage", "undo", "redo"].forEach((name) => { chain[name] = vi.fn(() => chain); });
  chain.run = vi.fn(() => true);
  const editor = { chain: vi.fn(() => chain), isActive: vi.fn(() => false), can: vi.fn(() => ({ undo: () => true, redo: () => false })), getAttributes: vi.fn(() => ({ href: "https://old.test" })), getHTML: vi.fn(() => "<p>old</p>"), commands: { setContent: vi.fn() }, setEditable: vi.fn() };
  return { editor, chain, useEditorMock: vi.fn(() => editor) };
});

vi.mock("@tiptap/react", () => ({
  useEditor: useEditorMock,
  useEditorState: ({ selector }: { selector: (value: { editor: typeof editor }) => unknown }) => selector({ editor }),
...
describe("RichTextEditor", () => {
  beforeEach(() => { vi.clearAllMocks(); useEditorMock.mockReturnValue(editor); editor.getHTML.mockReturnValue("<p>old</p>"); });

  it("runs every toolbar formatting command", () => {
    render(<RichTextEditor value="<p>old</p>" onChange={vi.fn()} />);
    ["Bold", "Italic", "Strikethrough", "Heading 2", "Heading 3", "Bullet list", "Numbered list", "Quote", "Code block", "Undo"].forEach((name) => fireEvent.click(screen.getByRole("button", { name })));
  ...
  it("sets, removes, and cancels links and images from prompts", () => {
    ...
    expect(chain.setImage).toHaveBeenCalledWith({ src: "https://img.test/a.png" });
    prompt.mockReturnValueOnce("");
    fireEvent.click(screen.getByRole("button", { name: "Add image" }));
  });

  it("syncs changed values/editability and renders a placeholder without an editor", () => {
    const { rerender } = render(<RichTextEditor value="<p>new</p>" editable={false} onChange={vi.fn()} />);
