...
const { authMock, routerMock, canReadMock, summaryMock, listMock, detailMock } =
  vi.hoisted(() => ({
    authMock: vi.fn(),
...
vi.mock("@/lib/postings/analytics", () => ({
  postingsAnalyticsApi: {
    getOwnerSummary: summaryMock,
    listOwnerPostings: listMock,
    getPostingDetail: detailMock,
  },
}));
...
vi.mock("./analytics-ui", () => ({
  ...
  LoadingDashboard: () => <div>Loading analytics</div>,
  ...
  StatCard: ({ eyebrow }: { eyebrow: string }) => <div>{eyebrow}</div>,
...
const derivedMetrics = {
  ctr: 20,
  viewToRequestRate: 10,
  requestToConfirmedRate: 50,
  utilizationRate: 25,
};
