...
vi.mock("@/components/auth/auth-captcha-panel", () => ({
  AuthCaptchaPanel: ({
    ...
    <div>
      ...
      <button type="button" onClick={() => onChange("fresh-token")}>
        Complete captcha
      </button>
...
describe("LoginUnlockPanel", () => {
  ...
  it.each([
    ...
    [
      clientError(429, "RATE_LIMITED", "Slow down", { retryAfterSeconds: 20 }),
      "A new unlock code was sent recently. Try again in 20 seconds.",
    ],
    [clientError(429, "RATE_LIMITED", "Please wait"), "Please wait"],
    [
      new Error("offline"),
      "We couldn't unlock sign-in right now. Please try again.",
    ],
  ...
  it.each([
    ...
    [
      clientError(400, "CAPTCHA_EXPIRED"),
    ...
    [
      clientError(429, "RATE_LIMITED", "Slow down", { retryAfterSeconds: 10 }),
      "A new unlock code was sent recently. Try again in 10 seconds.",
      null,
    ],
    [
      new Error("offline"),
  ...
  it("clears captcha field errors through both captcha callbacks", async () => {
    const user = userEvent.setup();
    resendUnlockLocalLoginMock.mockRejectedValue(
