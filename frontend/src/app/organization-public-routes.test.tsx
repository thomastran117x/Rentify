...
vi.mock("@/components/organizations/organization-public-detail-page", () => ({
  OrganizationPublicDetailPage: ({ id }: { id: string }) => (
    <div>Public {id}</div>
  ),
}));
...
describe("public organization routes", () => {
  ...
  it("renders public detail and blog views using the resolved organization", async () => {
    ...
    render(
      await OrganizationDetailPage({
        params: Promise.resolve({ id: "studio" }),
      }),
    );
    render(
      await OrganizationBlogRoute({
        params: Promise.resolve({ id: "studio" }),
      }),
    );
    ...
    expect(resolvePageMock).toHaveBeenCalledWith(
      "studio",
      "blog",
      "opening-day",
    );
  ...
  it("creates canonical metadata and safely falls back when resolution fails", async () => {
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => ({
        data: { title: "Opening day", excerpt: "Welcome" },
      }),
    });
    vi.stubGlobal("fetch", fetchMock);

    await expect(
      detailMetadata({ params: Promise.resolve({ id: "studio" }) }),
    ).resolves.toMatchObject({
      title: "Studio Co | Rentify",
      alternates: { canonical: "/organizations/studio" },
    });
    await expect(
      blogMetadata({ params: Promise.resolve({ id: "studio" }) }),
    ).resolves.toMatchObject({
      title: "Blog | Studio Co | Rentify",
      alternates: { canonical: "/organizations/studio/blog" },
    });
    await expect(postMetadata({ params })).resolves.toMatchObject({
      title: "Opening day | Rentify",
      alternates: { canonical: "/organizations/studio/blog/opening-day" },
    });
