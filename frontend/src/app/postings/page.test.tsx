...
vi.mock("@/components/postings/posting-result-card", () => ({ PostingResultCard: () => <article>Posting result</article> }));
vi.mock("@/components/postings/save-posting-button", () => ({ SavePostingButton: () => <button>Save</button> }));
vi.mock("@/components/postings/posting-autocomplete-input", () => ({ PostingAutocompleteInput: () => <input aria-label="Search rentals" /> }));
vi.mock("@/components/postings/organization-filter-field", () => ({ OrganizationFilterField: () => <input aria-label="Organization" /> }));

describe("PostingsPage", () => {
  it("parses supported query parameters and renders an empty search result", async () => {
    searchMock.mockResolvedValue({
      postings: [], source: "database", organizationFilter: undefined,
      pagination: { page: 2, pageSize: 50, total: 0, totalPages: 0, hasNextPage: false, hasPreviousPage: true },
    });

    render(await PostingsPage({ searchParams: Promise.resolve({
      q: " camera ", sort: "newest", page: "2", pageSize: "50", family: "equipment", subtype: "camera",
      tags: ["indoor,pro", "featured"], availabilityStatus: "available", minDailyPrice: "10", maxDailyPrice: "90",
      latitude: "43.7", longitude: "-79.4", radiusKm: "15", startAt: "2026-08-01", endAt: "2026-08-03",
    }) }));

    expect(searchMock).toHaveBeenCalledWith(expect.objectContaining({
      q: "camera", sort: "newest", page: 2, pageSize: 50, family: "equipment", subtype: "camera",
      tags: ["indoor", "pro", "featured"], availabilityStatus: "available", minDailyPrice: 10, maxDailyPrice: 90,
      latitude: 43.7, longitude: -79.4, radiusKm: 15,
    }));
    expect(screen.getByText("No postings matched your search. Try broadening your filters.")).toBeInTheDocument();
  ...
  it("renders a useful error when the search API fails", async () => {
    ...
    render(await PostingsPage({ searchParams: Promise.resolve({ sort: "not-a-sort", page: "0", pageSize: "7" }) }));

    expect(searchMock).toHaveBeenCalledWith(expect.objectContaining({ sort: "relevance", page: 1, pageSize: 20 }));
  ...
  it("groups organization-sorted results and reports a truncated organization match", async () => {
    searchMock.mockResolvedValue({
      postings: [
        { id: "posting-1", name: "Camera", organization: { id: "org-1", name: "Studio Co", slug: "studio-co" } },
      ],
      source: "elasticsearch",
      organizationFilter: { query: "studio", truncated: true, matches: [{ id: "org-1", name: "Studio Co" }] },
      pagination: { page: 1, pageSize: 20, total: 1, totalPages: 1, hasNextPage: false, hasPreviousPage: false },
    });

    render(await PostingsPage({ searchParams: Promise.resolve({ sort: "organizationAsc", organization: "studio" }) }));

    expect(screen.getByText("Studio Co")).toBeInTheDocument();
    expect(screen.getByText(/Many organizations matched that name/)).toBeInTheDocument();
  ...
  it("offers to clear an organization filter that has no matching organization", async () => {
    searchMock.mockResolvedValue({
      postings: [], source: "database", organizationFilter: { query: "missing", truncated: false, matches: [] },
      pagination: { page: 1, pageSize: 20, total: 0, totalPages: 0, hasNextPage: false, hasPreviousPage: false },
    });

    render(await PostingsPage({ searchParams: Promise.resolve({ organization: "missing" }) }));
