...
Use [pull-requests.md](./pull-requests.md) for the description format. Confirm the context, changes, API impact, reproducible consumer test workflow, and reviewer notes are accurate. For core frontend visual changes, inspect available screenshots and their state/viewport captions, or the stated capture limitation. Screenshots support review but do not establish behavioral correctness by themselves.
